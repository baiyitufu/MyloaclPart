...
//    [_videoPlayerView.shareBtn addTarget:self action:@selector(shareAction) forControlEvents:UIControlEventTouchUpInside];
    ...
    [_videoPlayerView.barTopView.JuJiButton addTarget:self action:@selector(showPlayList) forControlEvents:UIControlEventTouchUpInside];
    [_videoPlayerView.barTopView.goBackButton addTarget:self action:@selector(fullScreenButtonHandler) forControlEvents:UIControlEventTouchUpInside];
    [_videoPlayerView.CenterPlayBtn addTarget:self action:@selector(playBtnAction:) forControlEvents:UIControlEventTouchDown];

    
    
    if (self.videoPlayerView.playListView) {
        self.videoPlayerView.playListView.delegate=self;
    }
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch
{
    ...
    if ([touch.view isDescendantOfView:self.videoPlayerView.playerControlBar] || [touch.view isDescendantOfView:self.videoPlayerView.playListView]) {
        return NO;
    }
    return YES;
}
...
- (void)playVideoWithTitle:(NSString *)title URL:(NSURL *)url videoID:(NSString *)videoID shareURL:(NSURL *)shareURL isStreaming:(BOOL)streaming playInFullScreen:(BOOL)playInFullScreen
{
    ...
    if (streaming) {
          ...
          /*直播没有剧集 */
         self.videoPlayerView.barTopView.JuJiButton.hidden=YES;
...
-(void)changeViewAlphaWith:(float)altha{

    self.videoPlayerView.playerControlBar.alpha = altha;
    self.videoPlayerView.titleLabel.alpha = altha;
    _videoPlayerView.barTopView.alpha=altha;
    self.videoPlayerView.playListView.alpha=altha;
    
    self.videoPlayerView.actionViewZh.alpha=altha;
...
-(void)showPlayList{
    ...
    if (_videoPlayerView.playListView.hidden) {
       _videoPlayerView.playListView.hidden=NO;
    }else {
        _videoPlayerView.playListView.hidden=YES;
    }
...
-(void)playBtnAction:(id)sender{
    ...
    btn.hidden=YES;
     ...
     if (self.delegate&&[self.delegate respondsToSelector:@selector(playVideoBase:)]) {
        [self.delegate playVideoBase:btn];
    }
